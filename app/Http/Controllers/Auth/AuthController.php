...
class AuthController extends Controller
{
    ...
    public function postToken(Request $request)
    {
        ...
        if (! Authy::tokenIsValid($user, $request->token)) {
            return redirect()->to('login')->withErrors("2FA Token is invalid!");
        }
    ...
    public function postRegister(RegisterRequest $request, UserMailer $mailer, RoleRepository $roles)
    {
        ...
        if (settings('reg_email_confirmation')) {
            ...
            $message = 'You account is created successfully! Please confirm your email in order to log in.';
        } else {
            $message = "You account is created successfully! You can log in now.";
        }
